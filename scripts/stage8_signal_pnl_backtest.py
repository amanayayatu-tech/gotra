...
def select_mode() -> str:
    ...
    if full_exists and repro_exists and min(full_baseline, full_alaya, repro_baseline, repro_alaya) >= 1000:
        return "FULL"
    if full_exists and full_alaya >= 1000 and not repro_exists:
        return "FULL_PARTIAL"
...
def build_analysis(validation_status: str) -> dict[str, Any]:
    ...
    mode = select_mode()
    baseline_root = RUNS_ROOT / BASELINE_RUN
    ...
    baseline_coverage_ok = (
        field_probe["baseline_run"].get("n_steps", 0)
        and field_probe["baseline_run"].get("n_actual_non_null", 0)
        / field_probe["baseline_run"].get("n_steps", 1)
        >= 0.99
    )
    if mode == "ABORT" or not baseline_coverage_ok:
        return {
            "environment": environment,
            "mode": "ABORT",
            "field_probe": field_probe,
            "validation_status": validation_status,
            "error": "baseline actual coverage below 0.99 or no usable baseline run",
        }
        ...
        for cost_bps in COST_BPS_VARIANTS:
            ...
            if not reference_df.empty:
                results.append(
                    compute_strategy(
                        reference_df,
                        "reference_from_compare",
                        "reference_direction",
                        avoid_as_short,
                        cost_bps,
                    )
                )
                if reference_df["expected_change_pct"].notna().any():
                    results.append(
                        compute_strategy(
                            reference_df,
                            "reference_from_compare",
    ...
    primary_baseline = next(
        ...
        if result.label == "baseline_candidate"
        and result.direction_source == "decision_direction"
        and not result.avoid_as_short
        and result.cost_bps == PRIMARY_COST_BPS
    )
    primary_reference = next(
        result
        for result in results
        if result.label == "reference_from_compare"
        and result.direction_source == "reference_direction"
        and not result.avoid_as_short
        and result.cost_bps == PRIMARY_COST_BPS
    )
    primary_buy_hold = next(
        result
        for result in results
        if result.label == "buy_hold_equal_weight"
        and not result.avoid_as_short
        and result.cost_bps == PRIMARY_COST_BPS
    )
    ...
    for avoid_as_short in AVOID_AS_SHORT_VARIANTS:
        for cost_bps in COST_BPS_VARIANTS:
            ...
            random_blocks[(avoid_as_short, cost_bps)] = {
                "baseline_candidate": random_summary(
                    next(
                        result
                        for result in results
                        if result.label == "baseline_candidate"
                ...
                "reference_from_compare": random_summary(
                    next(
                        result
                        for result in results
                        if result.label == "reference_from_compare"
...
def render_report(analysis: dict[str, Any]) -> str:
    ...
    primary_reference: StrategyResult = analysis["primary_reference"]
...
主口径下 baseline candidate 随机百分位={random_primary["baseline_candidate"]["percentile"]:.1f}，reference 随机百分位={random_primary["reference_from_compare"]["percentile"]:.1f}。
...
2. 关键经济指标（主口径 AVOID_AS_SHORT=False, COST_BPS=10）：
   ...
   - baseline candidate：累计收益 {pct(primary_baseline.metrics["cumulative_return"])}，夏普 {num(primary_baseline.metrics["sharpe"])}, 最大回撤 {pct(primary_baseline.metrics["max_drawdown"])}。
   - reference_from_compare：累计收益 {pct(primary_reference.metrics["cumulative_return"])}，夏普 {num(primary_reference.metrics["sharpe"])}, 最大回撤 {pct(primary_reference.metrics["max_drawdown"])}。
   - buy & hold：累计收益 {pct(primary_buy_hold.metrics["cumulative_return"])}，夏普 {num(primary_buy_hold.metrics["sharpe"])}, 最大回撤 {pct(primary_buy_hold.metrics["max_drawdown"])}。
...
def summary_lines(analysis: dict[str, Any]) -> list[str]:
    ...
    mode = analysis["mode"]
    baseline_count = count_steps(BASELINE_RUN, "baseline")
    ...
    alaya_cov = smoke_probe.get("actual_coverage", "NA")
    ...
    return [
        f"[WORKTREE] branch={branch} HEAD={head} untracked={untracked or '[]'}",
        f"[MODE] {mode}",
        f"[DATA] baseline_runs={BASELINE_RUN}:{baseline_count} alaya_runs=stage7_full:missing,stage7_smoke:2",
        f"[FIELD_PROBE] baseline_actual_coverage={baseline_cov} "
        f"alaya_actual_coverage={alaya_cov} vote_consistency_available=no",
        "[PNL_AVOID_SHORT=False] "
        f"alaya: NA(BASELINE_ONLY) | baseline: {format_summary_metric(baseline_false)} | "
        f"reference: {format_summary_metric(reference_false)} | buy_hold: {format_summary_metric(buy_false)}",
        "[PNL_AVOID_SHORT=True]  "
        f"alaya: NA(BASELINE_ONLY) | baseline: {format_summary_metric(baseline_true)} | "
        ...
        "[VS_RANDOM] "
        "alaya_pctl=NA "
        f"baseline_pctl={random_false['baseline_candidate']['percentile']:.1f} "
        f"reference_pctl={random_false['reference_from_compare']['percentile']:.1f} "
        f"(N={RANDOM_MONTE_CARLO_N})",
        "[VERDICT] 数据不足以下真双臂判断（BASELINE_ONLY）；"
        "baseline/reference 只构成本地单臂经济筛查，不证明 alaya 机制经济价值",
        "[ZERO_FUTURE] declared=yes",
        "[KEY_TOUCHED] no",
        "[PUSHED] no",
        f"[ARTIFACTS] script=scripts/stage8_signal_pnl_backtest.py report={REPORT_PATH.as_posix()}",
