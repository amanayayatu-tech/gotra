...
- **输出侧**：解析时强制 ref 隔离（v2 `parse_provider_decision` 模式）：
  ...
  - provider prompt 必须显式写入上述 arm-specific ref 合同，并给出同 schema 的 JSON skeleton；该提示只用于提高 provider 合同遵循稳定性，不改变 schema 字段、指标或验收口径。
  - parser/schema guard 仍必须严格拒绝非法 refs；不得把非法 `ksana_refs` / `alaya_memory_refs` 静默清空后计入 scored。
