...
- **输出侧**：解析时强制 ref 隔离（v2 `parse_provider_decision` 模式）：
  ...
  - `full_gotra`：`alaya_memory_refs` 仅当每条 `availability_date <= decision_date` 才允许。
...
每个 full_gotra alaya_memory_ref 必须满足 availability_date <= decision_date。
...
```text
方法 1（点估计 + 时序稳健）：
  hac_mean_test(loss_diffs)   ← 复用 gotra/backtest/statistics.py::hac_mean_test
  （HAC 处理同 ticker 时间序列自相关；返回 mean_loss_diff / hac_lag / z_score / p_value）
...
聚类原则：
  ...
  HAC 以 (ticker 内按 decision_date 排序) 的损失差序列计算长程方差。
```
...
实现清单（全部带 focused tests，见 prereg §11）:
  ...
  2. gotra/backtest/statistics.py 新增（不改旧两臂函数）:
     - paired_loss_differences_v3(steps, left, right, *, input_layer=None, segment="scored")
     - cluster_bootstrap_ci(loss_diffs_by_ticker, *, iters=10000, seed, alpha=0.05)
